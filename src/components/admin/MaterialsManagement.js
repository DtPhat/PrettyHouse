...
const MaterialsManagement = () => {
  ...
  const hanldeDelete = () => {
    axiosJWT.delete(`https://kietpt.vn/api/material/${deleteId}`)
      .then(res => {
        handleClose()
      ...
      .catch((error) => {
        if (error.response && error.response.data && error.response.data.messageError) {
          ...
          toast.error("An error occurred while delete the materials.");
        }
        console.log(error);
      });
  }
  ...
  useEffect(() => {
    axiosJWT.get(`https://kietpt.vn/api/material/page/${currentPage}`).then(res => {
      setData(res?.data)
    })
  }, [currentPage]);
  console.log(data)
  return (
    <div>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", py: 2 }}>
        <Typography variant='h5' fontWeight={'700'}>Materials Management</Typography>
