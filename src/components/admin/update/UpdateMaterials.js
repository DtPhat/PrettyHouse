...
const UpdateMaterials = () => {
  ...
  const handleSubmit = () => {
    axiosJWT.put(`https://kietpt.vn/api/material/${id}`, formData)
      .then(res => {
      ...
      .catch((error) => {
        if (error.response && error.response.data && error.response.data.messageError) {
          ...
          toast.error("An error occurred while create the materials.");
        }
        console.log(error);
      });
  }
  ...
  useEffect(() => {
    axiosJWT.get(`https://kietpt.vn/api/material/${id}`).then(res => setFormData(res?.data?.data))
  }, []);
  return (
    <div>
      <Breadcrumbs separator=">" sx={{ paddingTop: "20px" }}>
        ...
        <Typography sx={{ fontSize: "20px" }}>Materials</Typography>
      </Breadcrumbs>
      <Typography variant='h5' fontWeight={700} sx={{pt:2}}>Update material</Typography>
      <Box
        component="form"
        sx={{
          display: 'flex',
