...
const UpdateMaterials = () => {
  ...
  const handleSubmit = () => {
    ...
    axiosJWT.put(`https://kietpt.vn/api/furniture/${id}`, submitData)
      .then(res => {
        console.log(res)
      ...
      .catch((error) => {
        if (error.response && error.response.data && error.response.data.messageError) {
          ...
          toast.error("An error occurred while update the furniture.");
        }
        console.log(error);
      });
  }
  ...
  useEffect(() => {
    ...
    axiosJWT.get(`https://kietpt.vn/api/furniture/ad/${id}`).then(res => {
      console.log("Previous data: ", res?.data?.data)
      const furnitureData = res?.data?.data
      console.log("Furniture Data: ", furnitureData)
      setFormData({
        name: furnitureData.name,
        description: furnitureData.description,
        image1: furnitureData.imgURL[0],
        image2: furnitureData.imgURL[1],
        image3: furnitureData.imgURL[2],
        colors: furnitureData.colors.map(item => item._id),
        materials: furnitureData.materials.map(item => item._id),
        sizes: furnitureData.sizes,
        price: furnitureData.price,
        returnExchangeCases: furnitureData.nonReturnExchangeCases.join('\n'),
        nonReturnExchangeCases: furnitureData.returnExchangeCases.join('\n'),
        delivery: "65c90357db6eeb89cdbe26aa",
        type: "DEFAULT",
        productClassification: furnitureData.classifications.filter(item => item.type == 'PRODUCT').map(item => item._id),
        styleClassification: furnitureData.classifications.filter(item => item.type == 'STYLE').map(item => item._id),
      })
    }).catch(error => {
      console.log(error)
    }).finally(() => {
  ...
  return (
    <div>
      ...
      <Typography variant='h5' fontWeight={700} sx={{ pt: 2 }}>Update Furniture</Typography>
      {
        loading
          ? <Loader />
          : <Box
